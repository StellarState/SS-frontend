...
interface FundingProgressTrackerProps {
  invoiceId: string;
  /** Static fallback values (shown before first poll completes). */
  target: number;
  raised?: number | null;
...
function Countdown({ deadline }: { deadline: string }) {
  ...
  useEffect(() => {
    function tick() {
      ...
      if (days > 0) setRemaining(`${days}d ${hours}h left`);
      else if (hours > 0) setRemaining(`${hours}h ${mins}m left`);
  ...
  return (
    <span className={`text-sm font-medium ${expired ? "text-red-500" : "text-muted-foreground"}`}>
      {remaining || "—"}
    </span>
  );
}
...
/**
 * #285 — Live funding progress tracker.
 * Polls every 30 s via React Query, shows percentage, raised/target,
 * investor count, and a countdown to the funding deadline.
 */
export function FundingProgressTracker({
  invoiceId,
  target,
  raised: raisedProp,
  investorCount: countProp,
  deadline,
}: FundingProgressTrackerProps) {
  const { data } = useQuery({
    queryKey: ["invoice-funding", invoiceId],
    ...
    <div className="space-y-3" data-testid="funding-progress-tracker">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm text-muted-foreground">
          {formatAmount(raised)} XLM of {formatAmount(target)} XLM
        </p>
      ...
      {deadline && new Date(deadline).getTime() < Date.now() && !isFullyFunded && (
        <div className="rounded-md bg-red-50 p-2 text-center text-sm font-medium text-red-700">
